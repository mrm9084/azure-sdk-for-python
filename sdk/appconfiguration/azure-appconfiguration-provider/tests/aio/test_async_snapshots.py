...
class TestSnapshotProviderIntegration(AppConfigTestCase):
    ...
    async def test_load_provider_with_snapshot_not_found(self, appconfiguration_endpoint_string):
        """Test loading provider with a non-existent snapshot returns error."""
        # Try to load from a non-existent snapshot
        provider = await self.create_client(
    ...
    @AppConfigProviderPreparer()
    @recorded_by_proxy_async
    async def test_load_provider_with_regular_selectors(self, appconfiguration_endpoint_string):
    ...
    async def test_create_snapshot_and_load_provider(self, appconfiguration_endpoint_string, **kwargs):
        ...
        async with await self.create_client(
            ...
            # Get the underlying SDK client from the provider
            sdk_client = provider._replica_client_manager._original_client._client
            ...
            try:
                ...
                # Restore modified settings to original values for other tests
                # Must happen inside async with so sdk_client is still open
                restored_settings = [
                ...
                await set_test_settings_async(sdk_client, restored_settings)
    ...
    async def test_create_snapshot_and_load_provider_with_feature_flags(
        ...
        async with await self.create_client(
            ...
            try:
                ...
                # Restore modified feature flag to original value for other tests
                # Must happen inside async with so sdk_client is still open
                await set_test_settings_async(
                    sdk_client,
                    [FeatureFlagConfigurationSetting(feature_id="SnapshotOnlyFeature", enabled=True, label=NULL_CHAR)],
                )

        # Load a fresh provider to verify snapshot immutability
        async with await self.create_client(
            endpoint=appconfiguration_endpoint_string,
